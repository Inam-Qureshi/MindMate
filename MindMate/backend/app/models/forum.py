...
class ForumQuestion(Base, SQLBaseModel):
    ...
    # Foreign Keys (only columns that exist in database)
    forum_id = Column(UUID(as_uuid=True), ForeignKey("forums.id"), nullable=False, index=True)
    ...
    # specialist relationship removed - column doesn't exist in database
    ...
    @property
    def specialist_id(self) -> Optional[uuid.UUID]:
        return None
    ...
    # Compatibility property for specialist relationship
    @property
    def specialist(self):
        return None
