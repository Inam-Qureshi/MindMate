...
def _ensure_patient_access(current_user_data: dict) -> Patient:
    """Ensure the current user is an authenticated patient and return the patient model."""
    user_type = current_user_data.get("user_type")
    user = current_user_data.get("user")

    logger.debug(
        "Journal access check | user_type=%s | user_class=%s | user_id=%s",
        user_type,
        type(user).__name__ if user is not None else None,
        getattr(user, "id", None)
    )

    if user_type != "patient" or not isinstance(user, Patient):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Journal entries are available to patient accounts only"
        )
...
async def create_journal_entry(
    ...
    """Create a new journal entry"""
    try:
        user = _ensure_patient_access(current_user_data)
        ...
        db.rollback()
        logger.exception(
            "Failed to create journal entry for user %s: %s",
            current_user_data.get("user").id if current_user_data.get("user") else "unknown",
            str(e)
        )
        raise HTTPException(
